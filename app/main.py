...
class Settings(BaseSettings):
    model_name: str = Field(default="black-forest-labs/FLUX.1-schnell", description="Pretrained model identifier")
...
class TextToImagePipelineInput(BaseModel):
    ...
    guidance_scale: float = Field(default=0.0, ge=0.0, le=20.0, description="Guidance scale for model")
    num_inference_steps: int = Field(default=1, ge=1, le=100, description="Number of inference steps")
    max_sequence_length: int = Field(default=256, ge=1, le=512, description="Maximum sequence length")
    ...
    def check_multiple_of_8(cls, v: int, field) -> int:
        if v % 8 != 0:
            raise ValueError(f"{field.name} must be a multiple of 8 for stable diffusion models")
        return v
